# STAFF MASTER QA READINESS

Generated from uploaded frontend repo scan on 2026-05-29.

## Scope

This document is the strict QA checklist for the **Staff** role only. It is designed for DeepSeek v4 Flash or another AI agent to run focused QA without pretending that shallow source review equals runtime testing.
...
|---|---|---|
| `/staff/dashboard` | `StaffDashboardPage` | today queue, check-in actions, payment shortcut |
| `/staff/bookings` | `StaffBookingsPage` | today/all bookings, filters, check-in and undo check-in |
| `/staff/payments` | `StaffPaymentsPage` | for-payment queue, collect payment, waive PF |
| `/staff/bookings/:id` | `StaffBookingDetailPage` | booking detail, check-in, undo, payment confirm, waive, receipt |
| `/staff/walk-in` | `StaffWalkInPage` | patient search/create and walk-in booking |
| `/staff/patients` | `StaffPatientsPage` | patient list and search |
| `/staff/patients/:id` | `StaffPatientDetailPage` | patient detail and portal account creation |
| `/staff/doctor-status` | `DoctorStatusPage` | doctor availability and running-late status |
| `/staff/profile` | `StaffProfilePage` | profile update and change password |

## Staff API Surface Detected From Frontend
...
| `GET bookings/staff/all?page={page}&pageSize={size}` |
| `GET bookings/staff/for-payment?page={page}&pageSize={size}` |
| `GET bookings/{id}` |
| `PATCH bookings/{id}/check-in` |
| `PATCH bookings/{id}/undo-check-in` |
...
## Staff-Specific High-Risk Areas

Staff owns front-desk operations. Do not mark PASS unless queue/status/payment actions update immediately and persist.

High-risk checks:
- Check-in and undo check-in must update queue state across dashboard, booking list, and booking detail.
- Payment collection must update booking/payment status and produce receipt data.
- Waive payment must require/submit reason and remove item from payment queue when expected.
- Walk-in booking must create/select patient, select doctor/service/slot, create booking, and show it in staff bookings.
- Doctor status changes must appear in public/booking doctor availability views.
- Patient portal account creation must produce a usable login or clear backend response.


## Required Staff QA Passes
