...
@Component({
  ...
  `,
  styleUrl: '../../staff/walk-in/staff-walk-in.page.scss'
})
export class WalkInPage implements OnInit {
  ...
  canAccessStep(step: WalkInStep): boolean {
    switch (step) {
      case 1: return true;
      case 2: return Boolean(this.selectedPatient);
      case 3: return this.canSubmitBooking;
  ...
  isStepComplete(step: WalkInStep): boolean {
    switch (step) {
      case 1: return Boolean(this.selectedPatient);
      case 2: return Boolean(this.selectedPatient && this.selectedDoctor && this.selectedService && this.selectedSlot);
      case 3: return false;
      default: return false;
    }
  }
  ...
  goToStep(step: WalkInStep): void {
    if (this.canAccessStep(step)) this.currentWalkInStep = step;
  }
  ...
  patientAccountStatus(patient: WalkInPatient | null | undefined): 'LinkedAccount' | 'NoAccount' | 'AccountUnknown' {
    if (!patient) return 'AccountUnknown';
    if (patient.hasAccount === true || Boolean(patient.userId?.trim())) return 'LinkedAccount';
    if (patient.hasAccount === false) return 'NoAccount';
  ...
  patientAccountLabel(patient: WalkInPatient | null | undefined): string {
    switch (this.patientAccountStatus(patient)) {
      case 'LinkedAccount': return 'Account Linked';
      case 'NoAccount': return 'No Account';
      default: return 'Account Unknown';
    }
  }
  ...
  openQuickRegister(): void { this.showQuickRegister = true; }
  ...
  cancelQuickRegister(): void {
    ...
    this.quickRegisterForm.reset({
      ...
      contactNumber: '', email: '', address: '', preparePortalAccount: false
    });
  }

  retrySearch(): void { this.loadPatients(this.searchControl.value); }
  ...
  private searchPatients(query: string): void { this.loadPatients(query); }
  ...
  private loadServicesForDoctor(doctorId: string): void {
    ...
    this.apiService.get<any[]>('doctors/' + doctorId + '/services').pipe(
      ...
      finalize(() => { if (token === this.servicesRequestToken) this.isLoadingServices = false; }),
      takeUntilDestroyed(this.destroyRef)
    ).subscribe({
      next: (services) => {
        if (token !== this.servicesRequestToken) return;
  ...
  private refreshAvailableSlots(): void {
    ...
    this.apiService.get<any[]>('doctors/' + doctorId + '/available-slots?date=' + date).pipe(
      ...
      next: (slots) => {
        if (token !== this.slotsRequestToken) return;
  ...
  private refreshCurrentStep(): void {
    if (!this.selectedPatient) { this.currentWalkInStep = 1; return; }
    if (this.canSubmitBooking) { this.currentWalkInStep = 3; return; }
...
function mapAvailableSlot(slot: AvailableSlot): TimeSlot {
  ...
  const isAvailable = typeof slot.isAvailable === 'boolean' ? slot.isAvailable : typeof (slot as any).IsAvailable === 'boolean' ? (slot as any).IsAvailable : true;
  const isFull = capacity > 0 ? bookedCount >= capacity : false;
  return { time, endTime, status: isAvailable ? (isFull ? 'full' : 'available') : 'disabled' };
}
