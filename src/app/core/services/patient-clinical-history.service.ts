...
import { ConsultationRecordResponse } from './medical-records.service';

interface PatientClinicalHistoryBuildArgs {
  patientId: string;
  ...
  extraPrescriptions?: PatientClinicalHistoryDto['prescriptions'];
}

@Injectable({ providedIn: 'root' })
export class PatientClinicalHistoryService {
  buildPatientClinicalHistory(args: PatientClinicalHistoryBuildArgs): PatientClinicalHistoryDto {
    const { patientId, patientRow, bookingRows, records } = args;
    const extraPrescriptions = args.extraPrescriptions ?? [];
    ...
    const summary: PatientClinicalHistorySummaryDto = {
      ...
      activePrescriptions: records.prescriptions.length + extraPrescriptions.length,
    ...
    const historyConsultations: PatientClinicalHistoryConsultationDto[] = consultations.map((c) =>
      mapConsultation(c, bookingMap)
    );

    const dedupedPrescriptions = dedupePrescriptionEntries([
      ...records.prescriptions.map((item) => ({
